...
<div class="container-xxl">
    ...
    <div class="row align-items-center">
        <div class="border-0 mb-4">
            <div
                ...
                <h3 class="fw-bold mb-0">Property Edit</h3>
                <!--<button type="submit"-->
    ...
    <div class="card-body">
        <form method="post" enctype="multipart/form-data" action="{{ route('property.update',$data->id) }}">
            @csrf
            @method('PATCH')
                ...
                <div class="col-lg-12">
                    <div class="card mb-3">
                        <div class="card-header py-3 d-flex justify-content-between bg-transparent border-bottom-0">
                            <h6 class="mb-0 fw-bold ">Property Details</h6>
                        </div>
                        <div class="card-body">
                                            ...
                                            <select name="category_id" class="form-control">
                                                <option value="">Select Category</option>
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}" {{ $category->id == $data->category_id ? 'selected' : '' }}>{{ $category->category_title }}</option>
                                                    @endforeach
                                            </select>
                                    </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="title" name="title" class="form-control"
                                        value="{{ $data->title }}" placeholder="Property Title">
                                    @if ($errors->has('title'))
                                    ...
                                    <div class="col-md-6">
                                        @if(!empty($data->image) && is_array($data->image))
                                            @foreach($data->image as $img)
                                                <img 
                                                    src="{{ asset('public/PropertyImage/'.$img) }}" 
                                                    alt="Property Image"
                                                    style="width:140px; height:auto; border:1px solid #ddd; padding:6px; margin:5px;">
                                            @endforeach
                                        @else
                                            <p style="margin-top:35px; color:#888;">No images uploaded</p>
                                        @endif
                                    </div>
                                </div>
